...
(async () => {
  ...
  // search: header panel suggestions, submit to shop, chip + sidebar box
  await page.click('[data-search-toggle]'); await page.waitForTimeout(300);
  check(await page.locator('#site-search').isVisible() && (await page.evaluate(() => document.activeElement.id)) === 'search-input', 'search panel opens with focus in the input');
  await page.fill('#search-input', 'rose quartz'); await page.waitForTimeout(600);
  const sugg = await page.locator('.search__result').count();
  check(sugg >= 6 && (await page.locator('.search__result').first().textContent()).includes('Rose Quartz'), 'search suggestions for "rose quartz": ' + sugg);
  check((await page.locator('.search__result--guide').count()) === 1, 'search suggests the crystal guide entry');
  await page.keyboard.press('ArrowDown');
  check(await page.evaluate(() => document.activeElement.classList.contains('search__result')), 'arrow down moves into suggestions');
  await page.focus('#search-input'); await page.keyboard.press('Enter'); await page.waitForURL('**/shop.html?q=*'); await page.waitForLoadState('networkidle');
  const rqCount = products.searchProducts('rose quartz').length;
  check((await page.locator('.card').count()) === rqCount, 'shop shows ' + rqCount + ' search results');
  check((await page.locator('.chip').first().textContent()).includes('Search'), 'search chip shown');
  check((await page.inputValue('#filter-search')) === 'rose quartz', 'sidebar search box mirrors the query');
  check((await page.locator('[data-count]').textContent()).includes('rose quartz'), 'count mentions the query');
  await page.fill('#filter-search', 'xyzzy'); await page.waitForTimeout(400);
  check((await page.locator('.shop__empty').count()) === 1 && page.url().includes('q=xyzzy'), 'sidebar search: empty state + url');
  await page.click('.chip button[data-clear-search]'); await page.waitForTimeout(200);
  check((await page.locator('.card').count()) === products.products.length && !page.url().includes('q='), 'clearing the search chip restores all products');
  await page.goto(BASE + 'index.html', { waitUntil: 'networkidle' });
  ...
  await mp.goto(BASE + 'about.html', { waitUntil: 'networkidle' });
  await mp.click('[data-search-toggle]'); await mp.waitForTimeout(300);
  await mp.fill('#search-input', 'bracelet'); await mp.waitForTimeout(900);
  check((await mp.locator('.search__result').count()) >= 5, 'mobile search on a page without the catalogue loads it on demand');
  check(!(await mp.evaluate(() => document.documentElement.scrollWidth > window.innerWidth + 1)), 'mobile search panel causes no overflow');
  await mp.keyboard.press('Escape'); await mp.waitForTimeout(200);
  check(await mp.locator('#site-search').isHidden(), 'escape closes mobile search');
